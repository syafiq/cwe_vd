...
def training_function(model):
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(all_predictions, f"predictions_{shortname}_{cwe}_{epoch}.pt")
        torch.save(all_labels, f"labels_{shortname}_{cwe}_{epoch}.pt")

        eval_accuracy = metric_accuracy.compute(predictions=all_predictions, references=all_labels)["accuracy"]
        eval_f1 = metric_f1.compute(predictions=all_predictions, references=all_labels, average="macro")["f1"]
        precision = precision_score(all_labels.cpu(), all_predictions.cpu(), average="macro")
        recall = recall_score(all_labels.cpu(), all_predictions.cpu(), average="macro")
        ...
        #if dist.get_rank() == 0:
        #    model.module.save_pretrained(f"{shortname}_{cwe}_model_{epoch}")
        #    tokenizer.save_pretrained(f"{shortname}_{cwe}_model_{epoch}")
        model.save_pretrained(f"{shortname}_{cwe}_model_{epoch}")
        tokenizer.save_pretrained(f"{shortname}_{cwe}_model_{epoch}")
